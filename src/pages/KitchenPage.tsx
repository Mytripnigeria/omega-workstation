...
import { Clock, CheckCircle2, AlertCircle, ChefHat, GripVertical, RotateCcw, MessageSquare, AlertTriangle } from "lucide-react";
...
const KitchenPage = () => {
  ...
  const OrderCard = ({ order, showActions = true }: { order: Order; showActions?: boolean }) => {
    ...
    return (
      <div className={`bg-card border border-border rounded-xl p-3 border-l-4 ${getStatusBorderColor(order.status, remaining)}`}>
        <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            {order.status === "preparing" && getStaffDisplay(order) && (
              <Badge variant="outline" className="text-xs"><ChefHat className="w-3 h-3 mr-1" />{getStaffDisplay(order)}</Badge>
            )}
          </div>
        </div>
        ...
        <div className="space-y-2 mb-4">
          {order.items.map((item, idx) => (
            <div key={item.id} draggable={order.status === "preparing"} onDragStart={() => setDraggedItem({ orderId: order.id, itemId: item.id })} onDragOver={(e) => e.preventDefault()} onDrop={() => { if (draggedItem && draggedItem.orderId === order.id) { const fromIndex = order.items.findIndex((i) => i.id === draggedItem.itemId); reorderItems(order.id, fromIndex, idx); } setDraggedItem(null); }} className={`flex items-center gap-2 p-2 rounded-lg transition-all ${order.status === "preparing" ? "hover:bg-secondary/50 cursor-grab" : ""} ${item.completed ? "opacity-50" : ""}`}>
              {order.status === "preparing" && (
                <>
                  <GripVertical className="w-4 h-4 text-muted-foreground flex-shrink-0" />
                  <button onClick={() => toggleItemComplete(order.id, item.id)} className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors ${item.completed ? "bg-status-success border-status-success" : "border-muted-foreground hover:border-primary"}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <span className={`text-foreground truncate ${item.completed ? "line-through" : ""}`}>{item.name}</span>
        ...
        {showActions && (
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            {order.status === "new" && <Button size="sm" className="gradient-primary" onClick={() => updateOrderStatus(order.id, "preparing")}>Start Preparing</Button>}
            {order.status === "preparing" && <Button size="sm" variant="secondary" className="bg-status-success hover:bg-status-success/80 text-white" onClick={() => updateOrderStatus(order.id, "ready")}><CheckCircle2 className="w-4 h-4 mr-1" />Ready</Button>}
            {order.status === "ready" && <Button size="sm" variant="secondary" onClick={() => updateOrderStatus(order.id, "completed")}><CheckCircle2 className="w-4 h-4 mr-1" />Complete</Button>}
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-background p-3 sm:p-4 lg:p-6">
      <PageHeader title="Kitchen Display" icon={ChefHat} iconColor="text-category-lavender" badge={`${orders.length} Orders`} />

      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        {/* New Orders */}
        <div>
          <div className="flex items-center gap-2 mb-4"><div className="w-3 h-3 rounded-full bg-status-warning" /><h2 className="font-semibold text-foreground">New</h2><Badge variant="outline">{newOrders.length}</Badge></div>
          <div className="space-y-4">{newOrders.map((order) => <OrderCard key={order.id} order={order} />)}</div>
        </div>

        {/* Preparing */}
        <div>
          <div className="flex items-center gap-2 mb-4"><div className="w-3 h-3 rounded-full bg-status-process" /><h2 className="font-semibold text-foreground">Preparing</h2><Badge variant="outline">{onTimeOrders.length}</Badge></div>
          <div className="space-y-4">{onTimeOrders.map((order) => <OrderCard key={order.id} order={order} />)}</div>
        </div>

        {/* Delayed */}
        <div>
          <div className="flex items-center gap-2 mb-4"><div className="w-3 h-3 rounded-full bg-destructive" /><h2 className="font-semibold text-foreground">Delayed</h2><Badge variant="outline" className="text-destructive border-destructive">{delayedOrders.length}</Badge></div>
          <div className="space-y-4">{delayedOrders.length === 0 ? <div className="bg-card border border-border rounded-xl p-6 text-center text-muted-foreground"><CheckCircle2 className="w-8 h-8 mx-auto mb-2 opacity-50" /><p className="text-sm">No delayed orders</p></div> : delayedOrders.map((order) => <OrderCard key={order.id} order={order} />)}</div>
        </div>

        {/* Ready & Completed */}
        <div>
          <div className="flex items-center gap-2 mb-4"><div className="w-3 h-3 rounded-full bg-status-success" /><h2 className="font-semibold text-foreground">Ready</h2><Badge variant="outline">{readyOrders.length}</Badge></div>
              ...
              <div className="pt-4 border-t border-border">
                ...
                <div className="space-y-3">
                  {completedOrders.slice(0, 3).map((order) => (
                    <div key={order.id} className="bg-card/50 border border-border rounded-xl p-3 opacity-60">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2"><Badge variant="outline">Table {order.tableNumber}</Badge><span className="text-sm text-muted-foreground">{order.id}</span></div>
                        <Button size="sm" variant="ghost" onClick={() => recallOrder(order.id)}><RotateCcw className="w-4 h-4 mr-1" />Recall</Button>
                      </div>
                    </div>
                  ))}
                </div>
